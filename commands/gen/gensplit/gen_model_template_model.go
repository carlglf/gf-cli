...
import (
	...
	"r2game.com/libs/assert"
	"r2game.com/libs/lang"

	"r2game.com/libs/db/splittable"
...
var (
	...
	Table = "{TplTableName}"
	trueTable   = ""
	splitType   = "{TplSplitType}"
	splitFormat = "{TplSplitFormat}"
	splitKey    = "{TplSplitKey}"
	splitNum    = {TplSplitNum}
...
// GetModel 获取分表model
func GetModel(data ...interface{}) *arModel {
	defer clearTrueTable()

	if trueTable != "" {
		return Model
	}

	assert.Assert(len(data) > 0, assert.CODE_ERROR_CALL, lang.Lang("分表操作中必须带参数"))

	m := data[0].(map[string]interface{})
	val, ok := m[splitKey]
	assert.Assert(ok, assert.CODE_ERROR_CONFIG, lang.Lang("查询条件中必须有分表字段：%s", splitKey))
...
// 设置分表Model
func SetSplitValue(splitValue interface{}) {
	trueTable := splittable.GetSplitTable(Table, splitType, splitNum, splitFormat, splitValue)
	Model = &arModel{g.DB("{TplGroupName}").Table(trueTable).Safe()}
}

// clearTrueTable 每次调用结束清理掉TrueTable
func clearTrueTable() {
	trueTable = ""
}
